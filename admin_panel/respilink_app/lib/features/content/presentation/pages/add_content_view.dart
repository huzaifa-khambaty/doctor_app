...
class AddContentView extends StatefulWidget {
  ...
  const AddContentView({super.key, required this.onBack, this.existingContent});
...
class _AddContentViewState extends State<AddContentView> {
  ...
  late final _DescriptionController _descriptionCtrl;
  ...
  void initState() {
    ...
    _descriptionCtrl = _DescriptionController(rawHtml: c?.description ?? '');
    if (c != null) {
      ...
      _selectedSpecialtyIds =
          c.specialties?.map((s) => s.id).whereType<int>().toSet() ?? {};
  ...
  void _submit(String targetStatus) {
    ...
    if (_requiresFile &&
        _fileBytes == null &&
        (_existingFileUrl == null || _existingFileUrl!.isEmpty)) {
      SnackbarUtil.showSnackbar(
        context,
        message:
            'Please upload a ${_contentType == 'PDF' ? 'PDF' : 'video'} file',
        isError: true,
      );
      return;
    }
    ...
    final request = CreateContentRequest(
      ...
      description: _descriptionCtrl.isEmpty ? null : _descriptionCtrl.toHtml(),
      contentType: _contentType.toLowerCase(),
      link: null,
      externalLinks: externalLinks.isEmpty ? null : externalLinks,
      specialtyIds: _selectedSpecialtyIds.isEmpty
          ? null
      ...
      fileName: _fileName,
      thumbnailBytes: _thumbnailBytes != null
          ? List<int>.from(_thumbnailBytes!)
          : null,
    ...
    return _FormCard(
      ...
      child: Column(
        ...
        children: [
          ...
          _Label('Description *'),
          const SizedBox(height: 8),
          _BlockEditorField(controller: _descriptionCtrl),
        ],
      ),
    );
  }
  ...
  Widget _buildThumbnailZone() {
    ...
    // Show image preview when we have bytes or an existing URL
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          hasNew
                              ? (_thumbnailName ?? 'New image selected')
...
final class _TextEditorBlock extends _EditorBlock {
  ...
  _TextEditorBlock([String raw = '']) : ctrl = _RichTextController(raw);
}
...
class _DescriptionController extends ChangeNotifier {
  ...
  _DescriptionController({String rawHtml = ''}) {
    _loadFromHtml(rawHtml);
    if (_blocks.isEmpty) _blocks.add(_TextEditorBlock());
  }

  List<_EditorBlock> get blocks => List.unmodifiable(_blocks);

  bool get isEmpty =>
      _blocks.every((b) => b is _TextEditorBlock && b.ctrl.text.trim().isEmpty);
  ...
  void removeBlock(int blockIndex) {
    final removed = _blocks.removeAt(blockIndex);
    if (removed is _TextEditorBlock) removed.ctrl.dispose();
    // Merge two adjacent text blocks that are now neighbours
    if (blockIndex > 0 &&
        blockIndex < _blocks.length &&
        _blocks[blockIndex - 1] is _TextEditorBlock &&
        _blocks[blockIndex] is _TextEditorBlock) {
      final prev = _blocks[blockIndex - 1] as _TextEditorBlock;
      final next = _blocks[blockIndex] as _TextEditorBlock;
      final sep = prev.ctrl.text.isNotEmpty && next.ctrl.text.isNotEmpty
          ? '\n'
          : '';
      final merged = _TextEditorBlock(prev.ctrl.text + sep + next.ctrl.text);
      prev.ctrl.dispose();
      next.ctrl.dispose();
      _blocks
        ..removeAt(blockIndex)
        ..removeAt(blockIndex - 1)
        ..insert(blockIndex - 1, merged);
    }
    if (_blocks.isEmpty) _blocks.add(_TextEditorBlock());
    notifyListeners();
  }
  ...
  void dispose() {
    for (final b in _blocks) {
      if (b is _TextEditorBlock) b.ctrl.dispose();
    }
    super.dispose();
  }
}
...
class _RichTextController extends TextEditingController {
  ...
  _RichTextController([String raw = ''])
    : super(text: _isHtml(raw) ? _parseHtml(raw).$1 : raw) {
    if (_isHtml(raw)) _formats.addAll(_parseHtml(raw).$2);
    _prev = text;
    addListener(_sync);
  }
  ...
  void _adjustSpans(String old, String curr) {
    ...
    while (pfx < min && old[pfx] == curr[pfx]) {
      pfx++;
    }
    int sfx = 0;
  ...
  void applyFormat(_Fmt fmt) {
    ...
    final already = _formats.any(
      (f) => f.fmt == fmt && f.start <= s && f.end >= e,
    );
  ...
  bool isActive(_Fmt fmt) {
    ...
    return _formats.any(
      (f) => f.fmt == fmt && f.start <= sel.start && f.end >= sel.end,
    );
  }
  ...
  String toHtml() {
    ...
    for (final f in _formats) {
      for (
        int i = f.start.clamp(0, plain.length);
        i < f.end.clamp(0, plain.length);
        i++
      ) {
    ...
    while (i < plain.length) {
      ...
      int j = i + 1;
      while (j < plain.length && _eq(charFmts[j], fmts)) {
        j++;
      }
  ...
  TextSpan buildTextSpan({
    ...
    while (i < plain.length) {
      ...
      children.add(
        TextSpan(text: plain.substring(i, j), style: _applyFmts(style, fmts)),
      );
      i = j;
    }
  ...
  bool _eq(Set<_Fmt> a, Set<_Fmt> b) =>
...
class _BlockEditorField extends StatefulWidget {
  final _DescriptionController controller;
  const _BlockEditorField({required this.controller});

  @override
  State<_BlockEditorField> createState() => _BlockEditorFieldState();
}

class _BlockEditorFieldState extends State<_BlockEditorField> {
  _DescriptionController get _ctrl => widget.controller;
  int _activeBlockIdx = 0;

  _RichTextController get _activeTextCtrl {
    final blocks = _ctrl.blocks;
  ...
  Future<void> _pickImage(int afterBlockIndex) async {
    final result = await FilePicker.platform.pickFiles(
      type: FileType.custom,
      allowedExtensions: ['jpg', 'jpeg', 'png', 'gif', 'webp'],
      withData: true,
    ...
    _ctrl.insertImageAfter(afterBlockIndex, file.bytes!, mime);
  }
          ...
          child: Column(
            ...
            children: [
              ...
              for (int i = 0; i < blocks.length; i++)
                if (blocks[i] is _TextEditorBlock)
                  _buildTextBlock(i, blocks[i] as _TextEditorBlock)
                else
                  _buildImageBlock(i, blocks[i] as _ImageEditorBlock),
              _buildFooter(),
            ],
          ),
        );
            ...
            return Row(
              children: [
                ...
                _FmtBtn(
                  ...
                  ctrl: cc,
                  bold: true,
                  onTap: () => cc.applyFormat(_Fmt.bold),
                ),
                _FmtBtn(
                  label: 'I',
                  fmt: _Fmt.italic,
                  ctrl: cc,
                  italic: true,
                  onTap: () => cc.applyFormat(_Fmt.italic),
                ),
                _FmtBtn(
                  label: 'U',
                  fmt: _Fmt.underline,
                  ctrl: cc,
                  underline: true,
                  onTap: () => cc.applyFormat(_Fmt.underline),
                ),
                _ToolbarSep(),
                Tooltip(
                  message: 'Insert Image',
                  child: InkWell(
                    onTap: () => _pickImage(_activeBlockIdx),
                    borderRadius: BorderRadius.circular(6),
                    child: Container(
  ...
  Widget _buildTextBlock(int index, _TextEditorBlock block) {
    return Focus(
      onFocusChange: (hasFocus) {
        if (hasFocus) setState(() => _activeBlockIdx = index);
      },
      child: TextField(
        controller: block.ctrl,
        minLines: index == 0 ? 5 : 2,
        maxLines: null,
        style: const TextStyle(
          fontSize: 13,
          color: AppColors.textDark,
        ...
        decoration: InputDecoration(
          hintText: index == 0 ? 'Enter description...' : '',
          hintStyle: TextStyle(
            color: AppColors.textMuted.withValues(alpha: 0.5),
            fontSize: 13,
          ),
  ...
  Widget _buildFooter() {
    final totalChars = _ctrl.blocks.whereType<_TextEditorBlock>().fold(
      0,
      (sum, b) => sum + b.ctrl.text.length,
    );
    return Container(
      ...
      child: Row(
        children: [
          Text(
            'Select text to format  •  Use toolbar image icon to insert an image',
            style: TextStyle(
              fontSize: 11,
              color: AppColors.textMuted.withValues(alpha: 0.6),
            ),
