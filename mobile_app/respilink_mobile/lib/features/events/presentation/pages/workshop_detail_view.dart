                ...
                builder: (context, registerState) {
                  return BlocBuilder<EventDetailBloc, EventDetailState>(
                    builder: (context, state) {
                      ...
                      final joinLink = state.detail.event.externalJoinLink
                          ?.trim();
                      final isJoinLive =
                          state.detail.event.isLive &&
                          joinLink != null &&
                          joinLink.isNotEmpty;
  ...
  Future<void> _joinLive(BuildContext context, String url) async {
    try {
      ...
      if (uri == null || !uri.hasScheme) {
        SnackbarUtil.showSnackbar(
          message: 'Invalid live session link.',
          isError: true,
        );
        return;
      }
      ...
      if (!launched && context.mounted) {
        SnackbarUtil.showSnackbar(
          message: 'Unable to open the live session.',
          isError: true,
        );
      }
    } catch (_) {
